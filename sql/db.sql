...
create table utilisateur (
	...
	constraint fk_categorieutilisateur foreign key(id_catutil) references categorieutilisateur(id_catutil)
)ENGINE=INNODB DEFAULT CHARSET=utf8;
...
create table incident  (
	...
	constraint fk_catincident foreign key (id_catincident) references catincident (id_catindident)
	constraint fk_zone foreign key (id_zone) references zone(id_zone)
...
create table intervention (
	...
	constraint fk_categorieintervention foreign key (id_catinterv) references categorieintervention (id_catinterv)
	constraint fk_incident foreign key (id_incident) references incident (id_indident)
);